...
app.get('/api/projects', async (req, res) => {
  try {
    ...
    res.status(200).json(projects); // Return JSON response
  } catch (err) {
    console.error('Error fetching projects:', err);
    res.status(500).json({ error: 'Error fetching projects' }); // Error in JSON format
  }
});
...
app.get('/api/skills', async (req, res) => {
  try {
    ...
    res.status(200).json(skills); // Return JSON response
  } catch (err) {
    console.error('Error fetching skills:', err);
    res.status(500).json({ error: 'Error fetching skills' }); // Error in JSON format
  }
});
